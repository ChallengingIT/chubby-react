...
const TabellaCandidati = ({ idNeed }) => {
    ...
    const fetchMoreDataCandidati = async (paginaCandidati, filters = filtri) => {
        const url = `http://80.211.138.142:8443/need/react/match/associabili/ricerca/mod/${id}`
    ...
    const handleAssocia = async (row) => {
        try {
            ...
            const url = `http://80.211.138.142:8443/need/add/shortlist?id=${idNeed}&idCandidato=${idCandidato}&username=${username}`;
