...
const IntervistaBox = ({
  ...
  const [values, setValues] = useState({
    ...initialValues,
    dateOra: new Date().toISOString().slice(0, 16),
  });
        ...
        return (
          <Grid item xs={xs} sm={12} md={md} lg={lg} xl={xl} key={field.name}>
            <FormControl
              fullWidth
              key={field.name}
              sx={{ marginBottom: "1.5em", marginTop: "1.5em", flexGrow: 1 }}
            >
    ...
    >
      <form onSubmit={handleSubmit}>
        ...
        <Grid container spacing={3}>
          {groupedFields[currentGroupIndex]
            .slice(
              groupedFields[currentGroupIndex][0].type === "titleGroups" ? 1 : 0
            )
            .map((field, index) => (
              <Grid
                item
