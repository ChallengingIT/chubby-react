...
function Interviste() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://localhost:8443/intervista/react/${id}`);
  ...
  const handleDelete = async (idToDelete) => {
    try {
      const response = await axios.delete(`https://localhost:8443/intervista/react/elimina/${idToDelete}`);
