...
const FieldBoxFile = ({
    ...
    const handleDownloadCVCF = async (fileId, fileDescrizione) => {
        ...
        const handleDeleteCVCF = async (fileId, idCandidato, fileType) => {
            ...
            try {
            const response = await axios.delete(`http://89.46.67.198/files/react/elimina/file/candidato/${fileId}/${idc}`, { headers: headers })
            if(response.data === "OK") {
            } else {
        ...
        const handleDeleteAllegati = async (fileId, idStaff) => {
            try {
                const ids = idStaff; 
                const url = `http://89.46.67.198/files/react/elimina/file/${fileId}/${ids}`;
