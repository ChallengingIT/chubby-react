...
const Recruiting = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/staffing/elimina/${id}`);
      const updatedRecruiting = originalRecruiting.filter((recruiting) => recruiting.id !== id);
      setRecruiting(updatedRecruiting);
      setOriginalRecruiting(updatedRecruiting);
      setFilteredRecruiting(updatedRecruiting);
    } catch (error) {
