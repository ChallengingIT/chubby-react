...
function Interviste() {
  ...
  const { id, nome, cognome, nomeAzienda, idCandidato } = params;
  ...
  const fetchData = async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        const intervisteConId = response.data.map((interviste, index) => ({ ...interviste, id: index + 1 }));
  ...
  const navigateToAggiungiIntervista = () => {
    navigate("/intervista/aggiungi", { state: { intervisteData: recruitingData } });
  };
...
renderCell: (params) => (
  ...
  { field: "azioni",         headerName: "Azioni",          width: 800, renderCell: (params) => (
    <div>
      ...
      state={{ intervisteData: recruitingData }}
>
<VisibilityButton recruitingData={recruitingData} />
</Link>
<Link
    ...
    <div className="container">
      <div className="content">
        ...
        <div className="containerTitle">
                    <h1>{`Lista ITW di ${recruitingData.nome} ${recruitingData.cognome} `}</h1>
                </div>
