...
//solo quando il componente viene montato effettua la chiamata per prendere i dati del mese e dell'anno corrente
    useEffect(() => {
        ...
        axios.get(`http://89.46.67.198/timesheet/react/staff/${id}/${anno}/${mese}`, { headers: headers})
            .then(response => {
    ...
    useEffect(() => {
        const fetchPrimoTimesheet = async () => {
            try {
                const response = await axios.get(`http://89.46.67.198/timesheet/react/staff/primo/${id}`, { headers: headers });;
...
//chiamata per passare al mese o anno precedente
    const handlePrevMese = () => {
        ...
        axios.get(`http://89.46.67.198/timesheet/react/staff/precedente/${id}/${annoNumero}/${meseNumero}`)
            .then(response => {
                const timesheetConId = response.data.mese.days.map((timesheet) => ({...timesheet}));
                // Crea un oggetto che raggruppa le ore per progetto e giorno
                const projectsMap = timesheetConId.reduce((acc, current) => {
...
//chiamata per passare al mese o all'anno successivo
    const handleSuccMese = () => {
        ...
        axios.get(`http://89.46.67.198/timesheet/react/staff/successivo/${id}/${annoNumero}/${meseNumero}`)
            .then(response => {
                const timesheetConId = response.data.mese.days.map((timesheet) => ({...timesheet}));
                // Crea un oggetto che raggruppa le ore per progetto e giorno
                const projectsMap = timesheetConId.reduce((acc, current) => {
    ...
    useEffect(() => {
        ...
        const handleSubmitModal = async () => {
            ...
            try {
            const response = await axios.post(`http://89.46.67.198/timesheet/react/staff/aggiorna/${id}/${annoNumero}/${meseNumero}`, datiDaInviare);
            if (response.data === "OK") {
            setModalOpen(false); 
            fetchTimesheetData(); 
        ...
        const fetchTimesheetData = async () => {
            try {
                const response = await axios.get(`http://89.46.67.198/timesheet/react/staff/${id}/${annoNumero}/${meseNumero}`);
                const timesheetConId = response.data.mese.days.map((timesheet) => ({...timesheet}));
                // Crea un oggetto che raggruppa le ore per progetto e giorno
                const projectsMap = timesheetConId.reduce((acc, current) => {
                    const idProgetto = current.progetto.id;
        ...
        const handleResetModalFPM = async () => {
            ...
            try {
                const response = await axios.post(`http://89.46.67.198/timesheet/react/staff/cancella/${id}/${annoNumero}/${meseNumero}`, datiDaInviare);
                setModalOpen(false); 
                setDatiTimesheet(response.data);
                fetchTimesheetData();
            } catch (error) {
        ...
        const handleSubmit = async () => {
            try {
                const responseSubmitTimesheet = await axios.post(`http://89.46.67.198/timesheet/react/staff/salva/${id}/${annoNumero}/${meseNumero}`, { headers: headers });
