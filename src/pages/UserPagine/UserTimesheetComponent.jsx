...
const UserTimeSheetComponent = ({ anno, setAnno, mese, setMese, timesheetData, setTimesheetData, id, idProgetti }) => {
...
const fetchData = async () => {
  ...
  const requestParams = {
    username: username // Aggiungi l'username come parametro della richiesta
  };


  
  try {
    const response = await axios.get(`http://localhost:8080/timesheet/user/${anno}/${mese}`, {
      headers: headers,
...
const handleCloseAlert = (event, reason) => {
  ...
  if (newMonthDate.getFullYear() < primoTimesheet.getFullYear() || 
    ...
    try {
      ...
      const url = `http://localhost:8080/timesheet/react/user/precedente/${prevYear}/${prevMonth}`;
      ...
      console.error("username inviato: ", requestParams);
    }
  ...
  const handleNextMonth = async () => {
    ...
    try {
      ...
      const url = `http://localhost:8080/timesheet/react/user/successivo/${nextYear}/${nextMonth - 1}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = `http://localhost:8080/timesheet/react/user/salva/${id}/${annoCorrente}/${meseCorrente}`;


      const response = await axios.post(url, timesheetData, {
        headers: headers,
        params: requestParams // Includi i parametri della richiesta qui
      });
...
const handleSubmitModal = async () => {
  ...
  console.log("ID DEL PROGETTO SELEZIONATO :", projectId);
    ...
    try {
      const url = `http://localhost:8080/timesheet/react/user/aggiorna/${annoCorrente}/${meseCorrente}`;
...
<Grid container justifyContent="space-between" marginTop='5px' alignItems="flex-end" paddingLeft="20px" spacing={2}>
  {Object.values(uniqueProjects).map((project, index) => (
    <React.Fragment key={index}>
...
export default UserTimeSheetComponent;