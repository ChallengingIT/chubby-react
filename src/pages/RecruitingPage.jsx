...
import RicercaRecruiting                    from "../components/ricerche/RicercaRecruiting.jsx";
...
const RecruitingPage = () => {
  ...
  // Varianti di animazione per far apparire la tabella
    ...
    const fetchData = async (paginaCorrente = 0) => {
        setLoading(true);
        try {
            const response = await ApiService.request("getAllRecruiting", {},{},{},{ pagina: paginaCorrente, quantita });
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                    const responseTipologia = await ApiService.request("tipologieAziende");
                    const responseTipo = await ApiService.request("tipoRecruitingSelect");
  ...
  const handleDelete = async () => {
    ...
    await ApiService.request("deleteRecruiting", { deleteId: deleteId });
      setOpenDialog(false);
      fetchData();
      handleOpenSnackbar(t('Candidato eliminato con successo!'), 'success');
    } catch (error) {
...
const handleDownloadCV = async (idFile, fileDescrizione) => {
  try {
    const responseDownloadCV = await ApiService.request(
      'downloadCV', 
      { idFile }, 
      {}, 
      {}, 
...
const handleCheckedCF = (idCandidato, nomeCandidato, cognomeCandidato, file, dataNascita) => {
  ...
  const handleDownloadCF = async (idCandidato, nomeCandidato, cognomeCandidato, tipo) => {
    try {
      ...
      const responseDownloadCF = await ApiService.request(
        "downloadCF",
        { idCandidato }, 
        {}, 
        {}, 
      ...
      setLoadingCF(false);
      handleOpenSnackbar(t('Download completato con successo!'), 'success'); 

    } catch (error) {
      setLoadingCF(false);
      if (error.response && error.response.status === 401) {
        handleOpenSnackbar(t('Errore: accesso non autorizzato per il download del CF.'), 'error'); 
      } else {
        console.error("Si è verificato un errore durante il download del CF: ", error);
        handleOpenSnackbar(t('Errore durante il download del CF.'), 'error');
      }
    }
  };
...
const openStato = Boolean(anchorElStato);
    ...
    const handleOpenSnackbar = (message, type = "success") => {
        setSnackbarMessage(message);
  ...
  const handleUpdateStato = async () => {
    ...
    const queryParams = { stato: idStato };
  
    try {
      const responseUpdateStato = await ApiService.request(
        "updateStatoRecruiting",
        { idCandidato },  
        {},  
        {}, 
        queryParams
      ...
      if (responseUpdateStato.data === "ERRORE") {
        ...
        handleOpenSnackbar(t('Errore durante il salvataggio dell\'azienda.'), 'error'); 
        return;
    }
