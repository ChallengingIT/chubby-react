...
const AggiungiRecruiting = () => {
    ...
    const handleChangeCV = (files) => {
        setFileCV(files); 
        setValues(prevValues => ({
            ...prevValues,
            cv: files.map(file => ({ descrizione: file.name })) 
        }));
    };
    
    const handleChangeCF = (files) => {
        setFileCF(files);
        setValues(prevValues => ({
            ...prevValues,
            cf: files.map(file => ({ descrizione: file.name }))
        }));
    };
    ...
    const handleSubmit = async (values) => {
        ...
        if (!hasErrors) {
            try {
                ...
                const datiResponse = await ApiService.request("salvaCandidato", {}, values, {}, {skill: skills});
