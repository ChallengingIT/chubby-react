  ...
  useEffect(() => {
    const fetchAziendeOptions = async () => {
      try {
        ...
        const responseFornitore           = await axios.get("https://89.46.67.198:8443/fornitori/react", { headers: headers });
        const responseJobTitle            = await axios.get("https://89.46.67.198:8443/aziende/react/tipologia", { headers: headers });
        const responseTipologia           = await axios.get("https://89.46.67.198:8443/staffing/react/tipo", { headers: headers });
        const responseNeedSkill           = await axios.get("https://89.46.67.198:8443/staffing/react/skill", { headers: headers });
...
const handleSubmit = async (values, fileCV, fileCF) => {
  ...
  if (!hasErrors) {
      ...
      const datiResponse = await axios.post("https://89.46.67.198:8443/staffing/salva", values, {
      params: { skill: skills },
      headers: headers,
      });
...
try{
  ...
  Object.keys(values).forEach(key => {
    ...
    if( fileCV instanceof File) {
      ...
      const responseCV = await axios.post(`https://89.46.67.198:8443/staffing/react/staff/salva/file/${candidatoId}`, formDataCV, 
      {headers: headers});
  } 
}
} catch(error) {
...
if(fileCF) {
  if( fileCF instanceof File) {
      ...
      const responseCF = await axios.post(`https://89.46.67.198:8443/staffing/react/staff/salva/file/${candidatoId}`, formDataCF, {headers: headers});
  }
}
} catch(error) {
