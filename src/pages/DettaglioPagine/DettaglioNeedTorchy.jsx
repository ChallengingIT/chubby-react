...
//   const { valori = {} } = location.state || {};
  ...
  const initialValues = {
    ...
    week:                       valori.week                                               || "",
    tipologia:                  valori.tipologia && valori.tipologia.descrizione        || "",
    tipo:                       valori.tipo                                               || "",
    owner:                      valori.owner     && valori.owner.descrizione            || "",
    stato:                      valori.stato     && valori.stato.descrizione            || "",
    numeroRisorse:              valori.numeroRisorse                                      || "",
    location:                   valori.location                                           || "",
    skills:                    (valori.skills?.map(skill => skill?.id))                   || [],
    skills2:                   (valori.skills2?.map(skill => skill?.id))                  || [],
    anniEsperienza:             valori.anniEsperienza                                     || "",
    note:                       valori.note                                               || "",          
  };
  ...
  return (
    ...
    <Box sx={{ marginLeft: '2em'}}>
          ...
          <Typography variant="h4" component="h1" sx={{ margin: '30px', fontWeight: 'bold', fontSize: '1.8rem', color: '#00853C'}}>Dettaglio Need di {valori.cliente.denominazione}</Typography>
          <FieldsBox
          fields={fields}
...
export default DettaglioNeedTorchy;
