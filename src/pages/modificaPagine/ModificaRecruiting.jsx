...
import CustomDragAndDrop from '../../components/fields/CustomDragAndDrop';




const ModificaRecruiting = () => {
    ...
    useEffect(() => {
        ...
        try {
            const responseStaffing          = await ApiService.request("candidatoByID", {id: id});
            const responseStato             = await ApiService.request("statoRecruitingSelect");
            const responseJobTitle          = await ApiService.request("tipologieAziende");
            const responseTipologia         = await ApiService.request("tipoCandidaturaCandidato");
            ...
            const facoltaResponse           = await ApiService.request("facoltaCandidato");
            const livelloScolasticoResponse = await ApiService.request("livelloScolasticoCandidato");
            const funzioniAziendaliResponse = await ApiService.request("funzioniAziendaliCandidato");
            const ricercaResponse           = await ApiService.request("ricercaCandidato");
            const tipoResponse              = await ApiService.request("tipoRecruitingSelect");
    ...
    const fetchJobTitleOptions = async (funzioneAziendaleId) => {
        try {
          const response = await ApiService.request("jobTitleSelect", {funzioneAziendaleId: funzioneAziendaleId});
    ...
    const handleSubmit = async (values) => {
        ...
        if (!hasErrors) {
            try {
                ...
                const datiResponse = await ApiService.request("salvaCandidato", {}, transformedValues, {}, {skill: skills});
                ...
                try{
                    if (fileCV) {
                        ...
                        const responseCV = await ApiService.request(
                            "salvaCVCandidato",
                    ...
                    try{
                        ...
                        const responseCF = await ApiService.request(
                            "salvaCVCandidato",
                            { candidatoId: candidatoId },
                            formDataCF, 
                            { 'Content-Type': 'multipart/form-data' } 
      ...
      try {
        ...
        { label: t("Note"),                               name: "note",                       type: "note", maxLength: 8000                                                           },
            ...
            { 
              ...
              name: "cv", 
              type: "customMultiFile", 
              onChange: handleChangeCV 
          },
          { 
              label: t("Consultant File"), 
              name: "cf", 
              type: "customMultiFile", 
              ...
              <Box sx={{ ml: 15, mr: 15 }}>
                  <Grid container spacing={2}>
                      {groupedFields[currentPageIndex].map((field, index) => {
                          ...
                          } else if (field.type === 'customMultiFile') {
                              return (
                                  <Grid item xs={12} key={index}>
...
export default ModificaRecruiting;