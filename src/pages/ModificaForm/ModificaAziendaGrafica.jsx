...
const ModificaAziendaGrafica = () => {
    ...
    useEffect(() => {
        const fetchProvinceOptions = async () => {
            try {
                const responseProvince = await axios.get("http://80.211.138.142:8443/aziende/react/province", { headers: headers });
                //const responseOwner = await axios.get("http://80.211.138.142:8443/owner", { headers: headers });
                const responseAziende = await axios.get("http://80.211.138.142:8443/aziende/react/select", { headers: headers });
                const responseModifica = await axios.get(`http://80.211.138.142:8443/aziende/react/${id}`, { headers: headers });
                const tipoServizioResponse = await axios.get("http://80.211.138.142:8443/hiring/servizi", { headers: headers });
    ...
    //funzione per capire quali campi sono obbligatori nel form corrente
            ...
            try {
                ...
                try {
                    if (fileIMG) {
                        ...
                        const responseIMG = await axios.post(`http://80.211.138.142:8443/aziende/react/salva/file/${aziendaID}`, formDataIMG, {
                            headers: {
