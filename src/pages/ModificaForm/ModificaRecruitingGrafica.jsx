...
const ModificaRecruitingGrafica = () => {
    ...
    useEffect(() => {
        const fetchAziendeOptions = async () => {
            try {
                const responseStaffing = await axios.get(`http://80.211.138.142:8443/staffing/react/${id}`, { headers: headers });
                const responseStato = await axios.get("http://80.211.138.142:8443/staffing/react/stato/candidato", { headers: headers });
                const responseJobTitle = await axios.get("http://80.211.138.142:8443/aziende/react/tipologia", { headers: headers });
    ...
    const handleDownloadCVCF = async (fileId, fileDescrizione) => {
        const url = `http://80.211.138.142:8443/files/react/download/file/${fileId}`;
        try {
    ...
    const handleSubmit = async (values) => {
        ...
        if (!hasErrors) {
            try {
                ...
                const datiResponse = await axios.post("http://80.211.138.142:8443/staffing/salva", transformedValues, {
                    params: { skill: skills },
                    headers: headers,
                });
                ...
                try {
                    if (fileCV) {
                        ...
                        const responseCV = await axios.post(`http://80.211.138.142:8443/staffing/react/staff/salva/file/${candidatoId}`, formDataCV,
                            { headers: headers });
                    }
                } catch (error) {
                ...
                try {
                    if (fileCF) {
                        ...
                        const responseCF = await axios.post(`http://80.211.138.142:8443/staffing/react/staff/salva/file/${candidatoId}`, formDataCF, { headers: headers });
                    }
                } catch (error) {
    ...
    const handleDeleteCVCF = async (fileId, fileType) => {
        ...
        try {
            const response = await axios.delete(`http://80.211.138.142:8443/files/react/elimina/file/candidato/${fileId}/${id}`, { headers: headers })
            if (response.data === "OK") {
            } else {
